...
export default function ScrollEffects() {
  useEffect(() => {
    ...
    /* Scroll progress */
    const progressBar = document.querySelector(".scroll-progress") as
      | HTMLDivElement
      | null;
    ...
    function parallaxFrame() {
      ...
      if (promoBg) {
        ...
        if (promo) {
          ...
          (promoBg as HTMLElement).style.transform = `translateY(${center * 0.12}px) scale(1.05)`;
        }
      }
    ...
    const revealSelectors =
      ".reveal, .reveal-left, .reveal-right, .menu-card, .gallery-item, .stat-item";
    ...
    document.querySelectorAll(revealSelectors).forEach((el, i) => {
      ...
      if (
        elem.classList.contains("menu-card") ||
        elem.classList.contains("gallery-item")
      ) {
        elem.dataset.delay = String((i % 6) * 80);
      }
